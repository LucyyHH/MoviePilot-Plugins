...
class MediaRecognizePreprocessParser:
    ...
    def _apply_rule(self, source: MetaSource, rule: RuleDef) -> Optional[ParseResult]:
        ...
        return ParseResult(
            ...
            season=self._to_int(season) if season is not None else rule.season,
            episode=self._to_int(episode) if episode is not None else rule.episode,
            matched_by="rule",
    ...
    @staticmethod
    def _group_value(match: re.Match, group_name: Optional[Union[str, List[str]]]) -> Optional[str]:
        group_names = MediaRecognizePreprocessParser._coerce_group_names(group_name)
        if not group_names:
            return None
            ...
            for index, item in enumerate(items):
                ...
                parsed_rules.append(
                    RuleDef(
                        ...
                        title_group=self._coerce_group_names(item.get("title_group"), ["title"]),
                        year_group=self._coerce_group_names(item.get("year_group"), ["year"]),
                        season_group=self._coerce_group_names(item.get("season_group")),
                        episode_group=self._coerce_group_names(item.get("episode_group")),
                        strip_patterns=self._coerce_list(item.get("strip_patterns")),
                    )
                )
            if parsed_rules:
    ...
    @staticmethod
    def _coerce_group_names(value, default: Optional[List[str]] = None) -> List[str]:
        if value is None:
