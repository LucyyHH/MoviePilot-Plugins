...
_SAMPLE_DIRECTORY_RULES = (
    "/data/media/subscription/cloudpan189-share => movie_path_rules, variety_issue_merge_rules, variety_path_rules, tv_path_rules, movie_year_prefix"
)
...
_SAMPLE_RULE_SETS = json.dumps(
    {
        "tv_path_rules": [
            {
                ...
                "pattern": (
                    r"^(?:"
                    r".*/(?P<title_series>[^/]+?)系列/(?:\d+[.\s_-]+)?[^/（）()]*?S(?P<season_series>\d{1,2})[.\s_-]+(?P<year_series>20\d{2})(?:[^/]*)/(?:[^/]*/)*(?:(?:[^/]*?S(?P<season_sxe_series>\d{1,2})[.\s_-]*E(?P<episode_sxe_series>\d{1,4})(?=[.\s_-]|$))|(?!(?:19|20)\d{2}[.\s_-])(?P<episode_series>\d{1,4})(?=[.\s_-]|$))"
                    r"|.*/(?:[A-Za-z]\s+|\d+[.\s_-]+)?(?P<title_year>[^/（）()]+?)(?:S(?P<season_title>\d{1,2}))?(?:\s*[（(](?P<year_paren>20\d{2})(?:[^）)]*)[）)]|[.\s_-]+(?P<year_dot>20\d{2}))(?:[^/]*)/(?:[^/]*/)*(?:(?:[^/]*?S(?P<season_sxe>\d{1,2})[.\s_-]*E(?P<episode_sxe>\d{1,4})(?=[.\s_-]|$))|(?:S(?P<season>\d{1,2})[.\s_-]*)?(?!(?:19|20)\d{2}[.\s_-])(?P<episode>\d{1,4})(?=[.\s_-]|$))"
                    r"|.*/(?P<title_parent>(?!(?:4K|2160P|1080P|720P)/)[^/（）()]+?)(?:\s+\d{1,4}(?:之后|以后|起|后续|续更|更新中))?/(?:(?:4K|2160P|1080P|720P)/)?(?P<episode_parent>\d{1,4})(?:\s*(?:4K|2160P|1080P|720P))?$)"
                ),
                "type": "tv",
                "use_path": True,
                "high_risk_mode": "warn",
                "cleanup_profile": "default_tv_cleanup",
                "title_group": ["title_series", "title_year", "title_parent"],
                "year_group": ["year_series", "year_paren", "year_dot"],
                "season_group": ["season_sxe", "season_sxe_series", "season", "season_series", "season_title"],
                "episode_group": ["episode", "episode_sxe", "episode_sxe_series", "episode_series", "episode_parent"],
                "strip_patterns": [
        ...
        "variety_path_rules": [
            {
                "name": "综艺-年份标题-S季目录-第N期分段",
                "pattern": r"^.*/(?P<title>[^/（）()]+?)[（(](?P<year>20\d{2})[）)]/S0?(?P<season>\d{1,2})[^/]*/\d?(?:20\d{2}[-_.]?\d{2}[-_.]?\d{2})[.\s_-]*第(?P<episode>\d+)\s*期(?:先导片上|先导片下|特别加更|还有加更|加更|上|中|下)?(?:[：:.\s_-].*)?$",
                "type": "tv",
                "use_path": True,
                "high_risk_mode": "warn",
                "cleanup_profile": "default_tv_cleanup",
        ...
        "variety_issue_merge_rules": [
            {
                "name": "综艺-目录标题-日期先导片",
                "pattern": r"^.*/(?P<title>[^/（）()]+?)/(?P<date>20\d{2}[-_.]?\d{2}[-_.]?\d{2})[.\s_-]*先导片(?:[：:.\s_-].*)?$",
                "type": "tv",
                "season": 1,
                "episode": 1,
                "use_path": True,
            ...
            {
                "name": "综艺-标题-S季目录-日期第N期",
                "pattern": r"^.*/(?P<title>[^/（）()]+?)/S0?(?P<season>\d{1,2})[^/]*/(?:(?P<date>20\d{2}[-_.]?\d{2}[-_.]?\d{2})[.\s_-]*)?(?:[^/]*?)第(?P<episode>\d{1,3})\s*期(?:\s*[（(]?[上下中][）)]?)?(?:[^/]*)?$",
                "type": "tv",
                "use_path": True,
                "high_risk_mode": "warn",
                "cleanup_profile": "default_tv_cleanup",
            ...
            {
                "name": "综艺-目录标题按期归并",
                "pattern": r".*/(?P<title>[^/（）()]+?)(?:\s*[（(]?(?P<year>20\d{2})(?:国综)?[）)]?|(?P<year_suffix>20\d{2}))?/(?:(?P<date>\d{8})[.\s_-]*)?(?:[^/]*?)第(?P<episode>\d{1,3})\s*期(?:\s*[（(]?[上下中][）)]?)?(?:[^/]*)?$",
                "type": "tv",
                "season": 1,
                "use_path": True,
                "high_risk_mode": "warn",
            ...
            {
                "name": "综艺-S季期数范围加更",
                "pattern": r".*/(?P<title>[^/（）()]+?)\s*S(?P<season>\d{1,2})\s*第\d{1,3}-(?P<episode>\d{1,3})期/(?:(?P<date>\d{8})[.\s_-]*)?(?:特别加更|还有加更|加更)(?:[：:.\s_-].*)?$",
                "type": "tv",
                "use_path": True,
                "high_risk_mode": "warn",
                "cleanup_profile": "default_tv_cleanup",
        ...
        "movie_path_rules": [
            {
                "name": "路径电影-书名号中文名括号年份",
                "pattern": r"^.*/《(?P<title>[^》]+)》[^/]*?[（(][^）)]*(?P<year>(19|20)\d{2})[^）)]*[）)].*$",
                "type": "movie",
                "use_path": True,
                "high_risk_mode": "warn",
            ...
            {
                "name": "路径电影-目录标题年份优先",
                "pattern": r"^(?!.*/[^/]*?(?:共\s*\d+\s*集|全\s*\d+\s*集|第\s*\d+\s*集|S\d{1,2}(?:[.\s_-]*E\d{1,4})?)[^/]*/).*/(?P<title>[^/（）()]+?)\s*[（(](?P<year>(19|20)\d{2})[）)](?:\s*[^/]*)?/(?:[^/]+)$",
                "type": "movie",
                "use_path": True,
                "high_risk_mode": "warn",
                "cleanup_profile": "default_movie_cleanup",
    ...
    [
        {
            ...
            "directories": [
                "/data/media/subscription/cloudpan189-share/百宝箱福利君/现在就出发3"
            ],
            "season": 3,
        ...
        },
        {
            ...
            "directories": [
                "/data/media/subscription/cloudpan189-share/更新超快/脱口秀和Ta的朋友们（2024）"
            ],
            "season": 2,
            "pattern": r"^\d?(?:(?P<date>20\d{2}[-_.]?\d{2}[-_.]?\d{2})[.\s_-]*)?第(?P<issue>\d+)\s*期(?P<part_tag>上|中|下)(?:[：:.\s_-].*)?$",
            "template": {
                "start_episode": 1,
                "issue_step": 2,
                "part_offsets": {
